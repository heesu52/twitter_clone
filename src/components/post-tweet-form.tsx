...
export default function PostTweetForm() {
    ...
    return (
        <Form onSubmit={onSubmit}>
            <TextArea required rows={5} maxLength={180} placeholder="What is happening?" value={tweet} onChange={onChange}></TextArea>
            <AttachFileButton htmlFor="file">
                {file? "Photo added" : "Add photo"}</AttachFileButton>
            <AttachFileInput type="file" id="file" accept="image/*" onChange={onFileChange}/>
            <SubmitBtn 
            type="submit" 
            value={isLoading ? "Posting..." : "post Tweet"}></SubmitBtn>
            </Form>
    )
}